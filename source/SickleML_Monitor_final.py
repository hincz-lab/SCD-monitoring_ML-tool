...
class CountAdheredBloodCells:
    ...
    def __init__(self, path, channel_filename):
        ...
        self.channel_image = cv.resize(self.channel_image, (150*self.beta, 150*self.alpha), interpolation = cv.INTER_CUBIC)
        self.resize_tile_width = int(self.channel_image.shape[1]/self.beta)
        self.resize_tile_height = int(self.channel_image.shape[0]/self.alpha)
    ...
    def process_tiles(self, kk=0):
        ...
        for ii in range(self.alpha):
            for jj in range(self.beta):
                y_slider, x_slider = ii*self.resize_tile_height, jj*self.resize_tile_width
    ...
    def preprocess_channel_mask(self, X, ensemble, kk=0):
        ...
        for ii in range(self.alpha):
            for jj in range(self.beta):
                ...
                pred_mask = cv.resize(pred_mask, (150,150), interpolation = cv.INTER_NEAREST)
...
#    Class methods for preparing the input data in Phase 2 is found below ...
    ...
    def ext_IndividualCells(self, channel_mask, padding = 20, crop_size = 32, count = 0):        
        ...
        centroids = [prop.centroid for prop in labelProperties if prop.area > 35]
...
#    Class methods for implementing Phase 2 is found below ...
    ...
    def count_classes(self, predictions):
       ...
       # sRBC_container, WBC_container, Other_container = [],[],[]
        for idx, pred_output in enumerate(predictions):
            pred_label = np.argmax(pred_output)
            if pred_label == 2:
                sRBC+=1
      #          sRBC_container.append(img_container[idx])
            if pred_label == 1:
                WBC+=1
       #         WBC_container.append(img_container[idx])
            if pred_label == 0:
                other+=1
     #           Other_container.append(img_container[idx])
    
        return sRBC,WBC,other    

    def count_classes_thres(self, predictions):
        sRBC, WBC, Other = 0, 0, 0
        sRBC_thres, WBC_thres, Other_thres = 0.5, 0.8, 0.9
        
        for sample in range(len(predictions)):
            if predictions[sample][2] >= sRBC_thres:
                sRBC+=1 
    
            if predictions[sample][1] >= WBC_thres:
                WBC+=1
            
            if predictions[sample][0] >= Other_thres:
                Other+=1
        return sRBC,WBC,Other
    ...
    def count_predictions(self, ensemble, img_container, phase=2):
        ...
        sRBC,WBC,Other = self.count_classes_thres(y_preds_Phase2)
        return sRBC, WBC, Other

#    Call the pipeline for cell counting ...

    # main function for cell counting
    def call_pipeline(self, Phase1_ensemble, Phase2_ensemble):
        ...
        sRBC, WBC, Other = self.count_predictions(Phase2_ensemble, img_container)
        print('Complete ...\n')
        return sRBC, WBC, Other
